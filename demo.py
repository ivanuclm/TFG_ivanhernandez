...
def main():
    ...
    if prediction:
        ...
        st.success(f'Predicción de popularidad: (MAE = 7.358) # {prediction:.2f}')
        st.progress(prediction/100)
        st.error(f'Predicción de popularidad básica: (MAE = 12.032) # {prediction_basic:.2f}')
        ...
        with aux2:  # Use the middle column for the button
            ...
            if st.button('Predecir popularidad de canción personalizada'):
                ...
                st.success(f'Predicción personalizada: (MAE = 7.358) # {user_prediction:.2f}')
                st.progress(user_prediction/100)
                st.error(f'Predicción personalizada básica: (MAE = 12.032) # {user_prediction_basic:.2f}')
